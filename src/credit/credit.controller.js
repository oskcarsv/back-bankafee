...
cron.schedule("59 * 23 * * *", async () => {
  ...
  for (const credit of creditsEndNow) {
    const accountOwner = await Account.findOne({
      noAccount: "GT16BAAFGTQ" + credit.no_Account_Owner,
    });
    ...
    if (accountOwner.amount - credit.creditAmount < 0) {
      ...
      await Account.findOneAndUpdate(
        { noAccount: "GT16BAAFGTQ" + credit.no_Account_Owner },
        { amount: accountOwner.amount - credit.creditAmount },
      );
    }
...
export const getCreditPetitions = async (req, res) => {
  ...
  const [total, credit] = await Promise.all([
    ...
    Credit.find(query).skip(Number(from)).limit(Number(limit)),
  ]);
