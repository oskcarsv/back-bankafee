...
export const putTransfer=async(req,res)=>{
    ...
    await Transfer.findByIdAndUpdate(transfer._id,{description:transfer.description});
    const transferNew = await Transfer.find({_id:idTransfer,status:'COMPLETED'});

    return res.status(200).json({msg:'Transfer updated  successfully',
