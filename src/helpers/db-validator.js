...
export const uniqueAccountInFavorites = async (noOwnerAccount, noAccount) => {
  ...
  const favoriteRecord = await Favorite.findOne({ noOwnerAccount: noOwnerAccount });

  if (!favoriteRecord) {
    throw new Error(`No favorite record found for owner account: ${noOwnerAccount}`);
  }

  const accountExistsInFavorites = favoriteRecord.favorites.some(favorite => favorite.noAccount === noAccount);

  if (accountExistsInFavorites) {
    throw new Error(`The account number ${noAccount} already exists in favorites.`);
  }
